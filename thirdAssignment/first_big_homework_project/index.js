...
const changePhone = (users) => {
  for (ele in users) {
      users[ele].phone = null;
  }
};
...
const c1 = getUserInfo("Rey.Padberg@karina.biz")
console.log(c1)
...
const changeName = (email, newUsername) => {
for (let i = 0; i < users.length; i++) {
  if (users[i].email === email) {
...
changeName("Telly.Hoeger@billy.biz","“Eren Yeager")
...
subArr = [50, 20, 40, 33, 60, 20, 90, 110, 15, 30];


const points = (arr1, arr2) => {
for (let i = 0; i < arr1.length; i++) {
  arr2[i].monthlyPoints = arr1[i]
}
}
...
const monthPoints = () => {
for (let i = 0; i < users.length; i++) {
  users[i].monthlyPoints = users[i].monthlyPoints * months
}
}
...
const addPoints = () => {
for (let i = 0; i < users.length; i++) {
  users[i].monthlyPoints = users[i].monthlyPoints * 1.1
} 
}
